...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, class_indices,
        ...
        selector_cls = getattr(importlib.import_module(name='exemplars_selection'), cls_name)
    ...
    def collect_exemplars(self, model, trn_loader):
        if self._is_active():
            self.images, self.labels = self.exemplars_selector(model, trn_loader)

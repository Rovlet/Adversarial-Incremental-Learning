...
from exemplars_selection import override_dataset_transform
...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=60, lr=0.1, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 momentum=0, wd=1e-5, multi_softmax=False, wu_nepochs=0, wu_lr_factor=1, fix_bn=False,
    ...
    def classify(self, task, features, targets):
        ...
        # Task-Aware Multi-Head zorientowany - patrzymy tylko na to, czy badana nowa klasa została zaklasyfikowana poprawnie
        num_cls = self.model.task_cls[task]
        offset = self.model.task_offset[task]
        pred = dists[:, offset:offset + num_cls]
        pred = pred.argmin(1)
        hits_taw = (pred + offset == targets.to(self.device)).float()
        # Task-Agnostic Multi-Head niezależny - patrzymy na wszystkie klasy - czy zostały zaklasyfikowane poprawnie
        pred = dists.argmin(1)
        hits_tag = (pred == targets.to(self.device)).float()
        return hits_taw, hits_tag, pred

    def compute_mean_of_exemplars(self, trn_loader):
        # change transforms to evaluation for this calculation
        with override_dataset_transform(self.exemplars_dataset) as _ds:
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        self.exemplars_dataset.collect_exemplars(self.model, trn_loader)
    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                if not self.exemplar_means:
                    hits_taw, hits_tag, pred = self.calculate_metrics(outputs, targets)
                else:
                    hits_taw, hits_tag, pred = self.classify(t, features, targets)
        ...
        return total_loss / total_num, total_acc_taw / total_num, total_acc_tag / total_num, predicted, true

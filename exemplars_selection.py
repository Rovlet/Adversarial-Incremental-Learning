...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader):
        clock0 = time.time()
        exemplars_per_class = self._exemplars_per_class_num(model)
        with override_dataset_transform(trn_loader.dataset, transform=None) as ds_for_selection:
            ...
            selected_indices = self._select_indices(model, sel_loader, exemplars_per_class)
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int) -> Iterable:
        pass
...
@contextmanager
def override_dataset_transform(dataset, transform=None):
    # if transform:
        # try:
